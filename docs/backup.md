...
Self-signed dev TLS certs auto-generated by `peershd` on first run under `%LOCALAPPDATA%\peersh\dev\`. The matching device id (`device_id = base32(sha256(publicKey)[:16])`) is what mobile clients use to address the host. **If you delete this directory, every mobile client's "Target device id" needs updating.**
...
### One-time: write your own `backup-notes.md`

Stash the things grep can't tell you from the file contents alone:

```
Firebase project id:           peersh-firebase-placeholder
Firebase Web API key:          (from app/lib/firebase_options.dart)
Cloud Run signaling URL:       wss://...
Cloud Run METRICS_TOKEN:       (from local/firebase_metrics_token.txt)
GCP billing account:           XXXXXX-XXXXXX-XXXXXX
Cloud Functions region:        asia-northeast1

Android debug keystore SHA-1:  ...
Android debug keystore SHA-256:...
Android release keystore SHA-1:...
Android release keystore SHA-256:...

OAuth Desktop client id:       ...apps.googleusercontent.com
OAuth Desktop client secret:   GOCSPX-...
```

Keep this file in the same encrypted backup as the keystores / secrets above.
