...
1. **Shell command execution.** From a paired mobile device, the user runs shell commands on a remote Windows PC or Mac and sees streamed `stdout` / `stderr` in real time, rendered with full xterm-style ANSI colours via xterm.dart. The remote shell is a real terminal attached to a pseudo-console — `pwsh` (or fallback `powershell.exe`) under ConPTY on Windows, the login shell (zsh/bash from `$SHELL`) under forkpty on macOS — not a sandboxed subset.
2. **Direct peer-to-peer transport.** All command and output traffic flows directly between the mobile device and the host (Windows PC or Mac) over a QUIC connection authenticated with mTLS. No third party can read it in transit.
...
4. On their Windows PC or Mac, they install and run `peershd`, configured to point at their signaling server's URL with their PSK credentials.
...
2. The user installs `peershd` on their Windows PC or Mac and the peersh APK on their phone.
...
1. A user opens the app on their phone. The Windows PC or Mac is asleep or has the network adapter idle.
...
4. The phone and Windows PC or Mac complete connection setup.
...
- **Linux hosts.** The host side runs on Windows and macOS; Linux hosts are not in scope.
- **Non-Windows, non-macOS shells.** The interactive shell is the host's own — PowerShell on Windows, the login shell (bash/zsh) on macOS. Shells on any other platform are out of scope.
