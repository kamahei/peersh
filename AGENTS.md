...
- **Cost discipline (Firebase mode).** ≤ ~5 Firestore reads + ~2 writes per connection lifecycle. No real-time listeners for signaling messages (Hello/Register/Connect/etc.). Wake-event listener on `users/{uid}/wake_requests` is permitted only on the host, implemented as a Realtime Database SSE stream (Firebase ID token + `?auth=`); both pair-code and service-account hosts use the same path so neither needs a persistent signaling WebSocket. Limit 1 listener per device, ~12 writes/hour heartbeat, ~1 read per wake delivery. Client-side caching for device info and public keys.
...
- **Cost discipline.** ≤ ~5 reads + ~2 writes per connection lifecycle. Client-side caching. No realtime listeners for signaling messages; the host runs a single Realtime Database SSE listener on `users/{uid}/wake_requests` instead of a persistent signaling WebSocket.
